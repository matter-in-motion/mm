...
Cluster.prototype.__initRequired = true;

Cluster.prototype.__init = function(units) {
...
Cluster.prototype.setupMaster = function() {
  const masterSettings = this.settings.master;
  const settings = {};
  if (masterSettings) {
...
Cluster.prototype.initListeners = function() {
  cluster.on('fork', worker => this.onWorkerFork(worker));
  cluster.on('online', worker => this.onWorkerOnline(worker));
  cluster.on('listening', (worker, address) => this.onWorkerListening(worker, address));
  cluster.on('disconnect', worker => this.onWorkerDisconnect(worker));
...
Cluster.prototype.onWorkerDisconnect = function(worker) {
  const info = this.workersInfo[worker.id];
...
Cluster.prototype.onWorkerExit = function(worker, code, signal) {
  ...
  cluster.fork();
  const logLevel = (worker.suicide ? null : 'fatal');
