...
Loader.prototype.run = function(cmd) {
  ...
  if (ctx) {
    if (typeof ctx === 'function') {
      ...
      w.runService.apply(w, this.args._);
    }
...
Loader.prototype.standardUnitPaths = {
  ...
  'core.transport.http': 'transport/http',
  'core.transport.ws': 'transport/ws'
};
...
Loader.prototype.loadUnit = function(name) {
  let Unit = this.resolveUnit(name);
  if (Unit) {
    return new Unit();
  }
};
