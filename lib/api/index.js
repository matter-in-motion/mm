...
Api.prototype.addResource = function(app, ns, api, ctrl) {
  ...
  Object.keys(api).forEach(method => {
    ...
    if (schema.request) {
      rpc.request = this.validator.addSchema(`resource.${name}:request`, schema.request);
    }

    if (this.debug && schema.response) {
      rpc.response = this.validator.addSchema(`resource.${name}:response`, schema.response);
    }
  });
...
Api.prototype.addDefinitions = function() {
  // body...
};

Api.prototype.addMethods = function() {
  // body...
};
...
Api.prototype.onCallResult = function(c, msg, result) {
  if (msg.connection.finished) {
    if (result !== undefined) {
      this.logger.warn({
        call: msg.call,
        result,
        sent: false
      }, `Response '${result}' to '${msg.call}'' can not be sent, connection is closed.`)
    }
    return;
  }
