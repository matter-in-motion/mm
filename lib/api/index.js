...
Api.prototype.unitInit = function(units) {
  ...
  this.slowResponse = settings.slowResponse;

  if (settings.websockets) {
    const ws = units.require('core.transport.ws');
...
Api.prototype.addProtocols = function(opts) {
  ...
  versions.forEach(version => {
    ...
    this.protocols[version] = protocol;
    this.use('/' + version, protocol.http.request(opts[version]));
...
Api.prototype.addHttpTransport = function() {
  this.options('/:version', (req, res) => this.handleHttpOptionsRequest(req, res));
  this.post('/:version', (req, res) => this.hahdleHttpRequest(req, res));
  return this;
};

Api.prototype.addWSTransport = function(ws) {
  this.ws = ws;
  ws.onMessage = (connection, message) => this.handleWSRequest(connection, message);
  return this;
};
...
Api.prototype.handleHttpOptionsRequest = function(req, res) {
  const msg = req.body;
  let call = msg.call;

  //return all resources
  if (!call) {
    return this.onSuccess(msg.id, res, Object.keys(this.schemas));
  }

  //return all methods
  if (this.schemas[call]) {
    return this.onSuccess(msg.id, res, Object.keys(this.schemas[call]));
  }
  ...
  return this.onSuccess(msg.id, res, this.schemas[call][method]);
};
...
Api.prototype.handleWSRequest = function(connection, message) {
  let version = connection.url.match(rxVersion);
  if (!version) {
    return; //skip, not an api message
  }
  ...
  const body = this.protocols[version].ws.request(connection, message);
  ...
  if (msg.head || c.auth.required ) {
    this.auth.verify(c.auth, msg.head, (err, auth) => {
      if (err && c.auth.required === true) {
        this.onAuthError(msg.id, connection);
      } else {
...
Api.prototype.callResource = function(c, auth, msg, connection, req) {
  ...
  if (c.raw) {
    return c.ctx[c.method](req || connection, auth, msg, (err, result) => this.onCallResult(id, connection, c, err, result));
  }
...
Api.prototype.onCallResult = function(id, connection, c, err, result) {
  if (err) {
    return this.onError(id, connection, err.code ? err : errors.Call(null, err.message));
  }
...
Api.prototype.send = function(data, connection) {
  if (this.slowResponse) {
    setTimeout(() => this._send(data, connection), randomDelay(this.slowResponse));
  } else {
    this._send(data, connection);
  }
};

Api.prototype._send = function(data, connection) {
  let logger = this.logger;
  this.protocols[connection.mmpVersion][connection.mmpTransport]
    .response(connection, data, err => {
      err && logger.error({
