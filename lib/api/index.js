...
Api.prototype.callResource = function(c, auth, msg) {
  if (c.raw || msg.raw) {
    const promise = c.method.call(c.ctx, msg.httpRequest || msg.connection, auth, msg.request, (err, result) => this.onCallResult(c, msg, err, result));
    return promise && this.onPromise(c, msg, promise);
  }
  ...
  const promise = c.method.call(c.ctx, auth, msg.request, (err, result) => this.onCallResult(c, msg, err, result));
  promise && this.onPromise(c, msg, promise);
};

Api.prototype.onPromise = function(c, msg, promise) {
  promise
    .then(result => this.onCallResult(c, msg, null, result))
    .catch(err => this.onCallResult(c, msg, err));
};
